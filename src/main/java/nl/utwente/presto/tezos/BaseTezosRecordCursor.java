...
public abstract class BaseTezosRecordCursor implements RecordCursor {
    protected final List<TezosColumnHandle> columnHandles;
    protected final int[] fieldToColumnIndex;

    protected List<Supplier> suppliers;

    public BaseTezosRecordCursor(List<TezosColumnHandle> columnHandles) {
        this.columnHandles = columnHandles;
        this.suppliers = Collections.emptyList();

        fieldToColumnIndex = new int[columnHandles.size()];
        for (int i = 0; i < columnHandles.size(); i++) {
            TezosColumnHandle columnHandle = columnHandles.get(i);
